...
public class DemoPushContentProvider implements CustomPushContentProvider {
    ...
    String imAccount = null;

    public DemoPushContentProvider() {
        imAccount = "user_" + DataSupport.findFirst(ResLoginInfoBean.class).getUserInfoId();
    }
    ...
    private String getContent(IMMessage message) {
        ...
        NimUserInfo userInfo = NimUserInfoCache.getInstance().getUserInfo(imAccount);
        if (userInfo == null) {
            NimUserInfoCache.getInstance().getUserInfoFromRemote(imAccount, null);
        }
