...
import com.fuyoul.sanwenseller.im.session.widgets.ToggleListener;
import com.fuyoul.sanwenseller.im.session.widgets.ToggleState;
import com.fuyoul.sanwenseller.im.session.widgets.ToggleView;
...
public class AVChatAudio implements View.OnClickListener, ToggleListener {
    ...
    public void showNetworkCondition(int grade) {
        if (grade >= 0 && grade < NETWORK_GRADE_DRAWABLE.length) {
            ...
            Drawable drawable = OkGo.getInstance().getContext().getResources().getDrawable(NETWORK_GRADE_DRAWABLE[grade]);
            if (drawable != null) {
    ...
    private void setWifiUnavailableNotifyTV(boolean show) {
        if (show && !NetworkUtil.isWifi(OkGo.getInstance().getContext())) {
