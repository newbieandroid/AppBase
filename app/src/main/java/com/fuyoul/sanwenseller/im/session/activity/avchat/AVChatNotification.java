    ...
    public void activeCallingNotification(boolean active) {
        if (notificationManager != null) {
            if (active) {
                ...
                ImConfigs.INSTANCE.getNotifications().put(CALLING_NOTIFY_ID, callingNotification);
            } else {
                notificationManager.cancel(CALLING_NOTIFY_ID);
                ImConfigs.INSTANCE.getNotifications().remove(CALLING_NOTIFY_ID);
    ...
    public void activeMissCallNotification(boolean active) {
        if (notificationManager != null) {
            if (active) {
                ...
                ImConfigs.INSTANCE.getNotifications().put(MISS_CALL_NOTIFY_ID, callingNotification);
            } else {
                notificationManager.cancel(MISS_CALL_NOTIFY_ID);
                ImConfigs.INSTANCE.getNotifications().remove(MISS_CALL_NOTIFY_ID);
            }
        }
    }
}
